...
General options:
  ...
  --token         API token (optional). If omitted, uses HF_TOKEN/CIVITAI_TOKEN from env.
...
if [[ -n "${COMFY_MODELS_DIR:-}" ]]; then
  models_dir="$(resolve_path "${COMFY_MODELS_DIR}")"
else
  models_dir="$comfy_dir/models"
fi

dest_dir="$models_dir/$model_dir"
mkdir -p "$dest_dir"
...
if [[ -z "$output_name" && -n "$file_path" ]]; then
