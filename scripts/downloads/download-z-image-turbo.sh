...
usage() {
  cat <<'EOF'
Download required models for Z-Image-Turbo into ComfyUI models folders.

Usage:
  ./scripts/downloads/download-z-image-turbo.sh [--force] [--token <hf_token>]
  ./scripts/downloads/download-z-image-turbo.sh -h | --help
...
if [[ -n "$explicit_token" ]]; then
  echo "[z-image-turbo] using explicit Hugging Face token"
elif [[ -n "${HF_TOKEN:-}" ]]; then
  echo "[z-image-turbo] using HF_TOKEN from environment"
else
  echo "[z-image-turbo] HF token not set; using anonymous download"
fi
...
download_hf_model() {
  ...
  if [[ -n "${requested_models[$model_key]:-}" ]]; then
    echo "[z-image-turbo] duplicate model entry skipped: $model_key"
    return 0
  fi
  ...
  if [[ -f "$destination" && "$force" != "true" ]]; then
    echo "[z-image-turbo] already exists, skipping: $destination"
    return 0
  fi
...
echo "[z-image-turbo] models root: $models_root"
